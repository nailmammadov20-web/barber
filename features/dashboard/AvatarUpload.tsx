  ...
  return (
    <div className="flex items-center gap-4">
      ...
      <button
        ...
        disabled={isPending}
        className={cn(
          "relative flex size-20 shrink-0 items-center justify-center overflow-hidden rounded-full border bg-primary text-2xl font-semibold text-primary-foreground transition-opacity",
          "hover:opacity-90 disabled:cursor-not-allowed"
        )}
      >
        {displayUrl ? (
          // eslint-disable-next-line @next/next/no-img-element
          <img src={displayUrl} alt={fullName} className="size-full object-cover" />
        ) : (
          fullName.charAt(0)
        )}
